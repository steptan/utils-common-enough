...
"""
    ...
    def test_prepare_lambda_buckets_success(self, deployer, mock_aws_clients):
        ...
        with patch.object(deployer, 'create_s3_bucket_if_needed', return_value=True) as mock_create:
    ...
    def test_deploy_success(self, deployer, mock_aws_clients):
        ...
        with patch.object(deployer, 'find_template', return_value=template_path):
            with patch.object(deployer, 'load_template', return_value=template_content):
                with patch.object(deployer, 'prepare_lambda_buckets', return_value=True):
                    with patch.object(deployer, 'deploy_stack', return_value=True):
                        with patch.object(deployer, 'wait_for_stack_complete', return_value=True):
                            with patch.object(deployer, 'get_stack_outputs', return_value={"ApiUrl": "https://api.example.com"}):
    ...
    def test_deploy_template_not_found(self, deployer):
        ...
        with patch.object(deployer, 'find_template', return_value=None):
            ...
            assert any("No CloudFormation template found" in err for err in (result.errors or []))
    ...
    def test_deploy_bucket_creation_failure(self, deployer):
        ...
        with patch.object(deployer, 'find_template', return_value=template_path):
            with patch.object(deployer, 'prepare_lambda_buckets', return_value=False):
                ...
                assert result.status == DeploymentStatus.FAILED
                assert any("Failed to create required S3 buckets" in err for err in (result.errors or []))
            ...
            with patch.object(deployer, 'load_template', return_value=template_content):
                with patch.object(deployer, 'prepare_lambda_buckets', return_value=True):
                    with patch.object(deployer, 'deploy_stack', return_value=False):
                        ...
                        assert result.status == DeploymentStatus.FAILED
                        assert any("Stack deployment failed" in err for err in (result.errors or []))
    ...
    def test_deploy_with_change_set(self, deployer, mock_aws_clients):
        ...
        with patch.object(deployer, 'find_template', return_value=template_path):
            with patch.object(deployer, 'load_template', return_value=template_content):
                with patch.object(deployer, 'prepare_lambda_buckets', return_value=True):
                    with patch.object(deployer, 'deploy_with_change_set', return_value=True):
                        with patch.object(deployer, 'wait_for_stack_complete', return_value=True):
                            with patch.object(deployer, 'get_stack_outputs', return_value={}):
                                result = deployer.deploy()
...
class TestBaseDeployer:
    ...
    def test_create_s3_bucket_if_needed_create(self, base_deployer):
        ...
        assert "CreateBucketConfiguration" not in call_args[1]  # us-east-1 doesn't need this
