...
class TestLambdaBuilder:
    ...
    def test_get_builder_for_runtime(self):
        ...
        node_builder = LambdaBuilder.get_builder_for_runtime(
            "nodejs18.x", Path("/tmp/func"), Path("/tmp/out")
        )
        ...
        with pytest.raises(ValueError, match="Unsupported runtime"):
            LambdaBuilder.get_builder_for_runtime(
                "python3.11", Path("/tmp/func"), Path("/tmp/out")
            )
...
class TestNodeJSBuilder:
    ...
    def test_install_dependencies_npm(self, builder):
        ...
        (builder.output_path / "package.json").write_text(
            json.dumps(
                {"name": "test-function", "dependencies": {"aws-sdk": "^2.1000.0"}}
            )
        )
    ...
    def test_build_typescript(self, builder):
        ...
        (builder.function_path / "tsconfig.json").write_text(
            json.dumps(
                {
                    "compilerOptions": {
                        "target": "es2020",
    ...
    def test_build_process(self, builder):
        ...
        # Setup
        builder.function_path.mkdir(parents=True)
        (builder.function_path / "index.js").write_text(
            "exports.handler = async () => {};"
        )
        (builder.function_path / "package.json").write_text(
            json.dumps(
                {"name": "test-function", "dependencies": {"aws-sdk": "^2.1000.0"}}
            )
        )
...
class TestLambdaPackager:
    ...
    def test_package_with_layers(self, packager):
        ...
        (packager.build_path / "node_modules" / "aws-sdk" / "index.js").write_text(
            "module.exports = {};"
        )
...
class TestLambdaBuilderIntegration:
    ...
    def test_build_nodejs_function(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (func_path / "package.json").write_text(
                json.dumps(
                    {
                        "name": "test-function",
                        "version": "1.0.0",
    ...
    def test_build_typescript_function(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (func_path / "tsconfig.json").write_text(
                json.dumps(
                    {
                        "compilerOptions": {
                            "target": "es2020",
