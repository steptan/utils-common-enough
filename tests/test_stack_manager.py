...
class TestStackManager:
    ...
    def test_get_stack_status_exists(self):
        ...
        manager.cloudformation.describe_stacks.assert_called_once_with(StackName="test-stack")
    ...
    def test_fix_rollback_state_with_skip_resources(self):
        ...
        with patch.object(manager, 'delete_stack', return_value=True):
            result = manager.fix_rollback_state("test-stack", skip_resources=["Resource1", "Resource2"])
            
            # Should continue rollback with skip resources
            manager.cloudformation.continue_update_rollback.assert_called_once_with(
                StackName="test-stack",
                ResourcesToSkip=["Resource1", "Resource2"]
            )
        ...
        list_versions_paginator.paginate.return_value = [
            {
                ...
                "DeleteMarkers": [
                    {"Key": "deleted.txt", "VersionId": "d1"}
                ]
            }
        ]
        
        manager.s3.get_paginator.side_effect = lambda x: (
            list_objects_paginator if x == "list_objects_v2" else list_versions_paginator
        )
    ...
    def test_delete_stack_force_with_delete_failed(self):
        ...
        with patch.object(manager, '_wait_for_deletion', return_value=True):
            ...
            # Should delete stack
            manager.cloudformation.delete_stack.assert_called_once_with(StackName="test-stack")
    ...
    def test_diagnose_stack_failure(self):
        ...
        # Check S3 failure
        s3_failure = next(r for r in diagnosis["failed_resources"] if r["resource_type"] == "AWS::S3::Bucket")
        assert "already exists" in s3_failure["reason"]
        
        # Check recommendations
        assert any("S3 bucket name already exists" in rec for rec in diagnosis["recommendations"])
        assert any("Check IAM permissions" in rec for rec in diagnosis["recommendations"])
