#!/usr/bin/env python3
"""
Add s3:PutBucketTagging permission to the CI/CD user.
This permission is required to create S3 buckets with tags.
"""
...
def update_policy_with_s3_permission(
    iam_client, policy_arn: str, policy_doc: Dict[str, Any]
) -> None:
    """Update the policy to include s3:PutBucketTagging permission."""
    ...
    if s3_statement:
        # Add s3:PutBucketTagging to existing S3 permissions
        ...
        added = []
        for perm in tagging_permissions:
        ...
        if added:
            ...
            print(f"✅ Added S3 tagging permissions: {', '.join(added)}")
        else:
            print("ℹ️  S3 tagging permissions already exist in policy")
            return
    else:
        # Create new statement for S3 permissions
...
def main():
    ...
    print(f"🔧 Adding S3 tagging permissions to people-cards CI/CD user...")
    ...
    # Update policy with S3 tagging permissions
    update_policy_with_s3_permission(iam, policy_arn, policy_doc)
