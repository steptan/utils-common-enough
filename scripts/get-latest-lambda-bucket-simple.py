...
def format_bucket_name(project: str, environment: str, thousands: int, number: int) -> str:
    """Format bucket name with given numbers."""
    return f"{project}-lambda-{environment}-{thousands:03d}-{number:03d}"

def list_project_buckets(s3_client, project_name: str, environment: str) -> List[Dict[str, any]]:
...
def main():
    ...
    if buckets:
        ...
        print(f"No Lambda buckets found for {project_name}-{environment}", file=sys.stderr)
