...
def fix_s3_permissions(iam_client, policy_arn: str, policy_doc: Dict[str, Any]) -> None:
    ...
    for statement in policy_doc["Statement"]:
        if statement.get("Sid") == "S3Permissions":
            ...
            if added:
                ...
                print(
                    f"✅ Added S3 tagging permissions to S3Permissions statement: {', '.join(added)}"
                )
            else:
                print(
                    "ℹ️  All required S3 tagging permissions already present in S3Permissions"
                )

            # Ensure resource is broad enough
            if statement["Resource"] != "*":
                print(
                    f"⚠️  S3Permissions Resource is limited to: {statement['Resource']}"
                )
                statement["Resource"] = "*"
    ...
    policy_doc["Statement"] = [
        s
        for s in policy_doc["Statement"]
        if s.get("Sid")
        not in ["UniversalTaggingPermissions", "AllPutTaggingPermissions"]
    ]
...
def main():
    ...
    print(
        "⏱️  Please wait 5-10 seconds for the policy to propagate before retrying deployment"
    )
