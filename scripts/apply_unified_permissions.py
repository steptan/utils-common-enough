...
class UnifiedPermissionManager:
    ...
    def update_user_with_unified_permissions(
        self,
        user_name: str,
        projects: List[str],
        ...
        try:
            ...
            self.iam.put_user_policy(
                UserName=user_name, PolicyName=policy_name, PolicyDocument=policy_json
            )
...
def apply(user: str, projects: tuple, region: str, profile: str, dry_run: bool):
    ...
    manager.update_user_with_unified_permissions(user, list(projects), region, dry_run)
        ...
        if policies["PolicyNames"]:
            ...
            for policy_name in policies["PolicyNames"]:
                policy_doc = manager.iam.get_user_policy(
                    UserName=user, PolicyName=policy_name
                )
