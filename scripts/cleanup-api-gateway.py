                    ...
                    try:
                        ...
                        for stage in stages.get("item", []):
                            if stage.get("stageName") == "api":
                                print(
                                    f"  - Found 'api' stage - this might be the orphaned resource"
                                )
...
def main():
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--help":
            print(
                "Usage: cleanup-api-gateway.py [--delete-stage-only] [--force-delete-api]"
            )
            print(
                "  --delete-stage-only: Only delete the 'api' stage, not the entire API"
            )
