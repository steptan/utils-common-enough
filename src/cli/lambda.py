...
from lambda.packager import LambdaPackager
...
@click.option('--source', '-s', required=True, type=click.Path(exists=True), help='Source directory containing Lambda code')
@click.option('--output', '-o', required=True, type=click.Path(), help='Output path for the deployment package')
@click.option('--runtime', '-r', required=True, type=click.Choice(['nodejs20.x', 'nodejs18.x', 'python3.11', 'python3.10', 'python3.9']), help='Lambda runtime')
@click.option('--handler', '-h', default=None, help='Lambda handler (e.g., index.handler or handler.lambda_handler)')
@click.option('--minify/--no-minify', default=True, help='Minify JavaScript code (Node.js only)')
@click.option('--requirements', type=click.Path(exists=True), help='Path to requirements.txt (Python only)')
...
@lambda_group.command()
@click.option('--package', '-p', required=True, type=click.Path(exists=True), help='Path to the Lambda deployment package')
...
@click.option('--source', '-s', type=click.Path(exists=True), help='Source directory (defaults to src/lambda)')
