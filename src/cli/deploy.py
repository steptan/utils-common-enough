...
@click.option('--build-env', '-E', multiple=True, help='Build environment variables (key=value)')
@click.option('--profile', help='AWS profile to use')
@click.option('--dry-run', is_flag=True, help='Show what would be deployed')
...
def full(project, environment, skip_build, profile, dry_run):
    ...
    try:
        ...
        if not infra_result.success:
            click.echo(f"❌ Infrastructure deployment failed: {infra_result.message}", err=True)
            sys.exit(1)
        ...
        if not frontend_result.success:
            click.echo(f"❌ Frontend deployment failed: {frontend_result.message}", err=True)
            sys.exit(1)
