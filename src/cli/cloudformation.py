...
@main.command()
@click.option('--stack-name', '-s', required=True, help='CloudFormation stack name')
@click.option('--force', '-f', is_flag=True, help='Force deletion, including cleanup of blocking resources')
@click.option('--region', help='AWS region')
@click.option('--profile', help='AWS profile to use')
...
def status(stack_name, project, region, profile, watch):
    ...
    try:
        ...
        if stack_name:
            ...
            for stack in stacks:
                status_color = 'green' if 'COMPLETE' in stack['status'] and 'ROLLBACK' not in stack['status'] else 'red' if 'FAILED' in stack['status'] else 'yellow'
                click.echo(f"{stack['name']:<40} {click.style(stack['status'], fg=status_color)}")
...
def get_output(project, environment, output_key, region, profile):
    ...
    try:
        ...
        if output_key in outputs:
            ...
            click.echo(f"Output '{output_key}' not found in stack {stack_name}", err=True)
            sys.exit(1)
