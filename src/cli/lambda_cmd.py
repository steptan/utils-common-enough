    ...
    try:
        ...
        if not runtime:
            if (function_path / "package.json").exists():
                ...
                click.echo(
                    "Error: Could not detect runtime. Please specify with --runtime",
                    err=True,
                )
                sys.exit(1)
...
def package(function: str, output: Optional[str], exclude: Tuple[str, ...]) -> None:
    ...
    try:
        ...
        result = packager.create_deployment_package(
            function_path, output_path, exclude_patterns=list(exclude)
        )
        ...
        for item in lambda_dir.iterdir():
            if item.is_dir() and not item.name.startswith("."):
                if (
                    ...
                    or (item / "index.ts").exists()
                    or (item / "index.js").exists()
                ):
