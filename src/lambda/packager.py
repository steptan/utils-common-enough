...
class LambdaPackager:
    ...
    def _create_zip(
        ...
        if handler_file:
            with zipfile.ZipFile(output_path, 'r') as zf:
                ...
                if handler_file not in files_in_zip and f"src/{handler_file}" not in files_in_zip:
                    print(f"⚠️  Warning: Handler file '{handler_file}' not found in package")
        
        print(f"✅ Created Lambda package: {output_path} ({output_path.stat().st_size / 1024 / 1024:.2f} MB)")
        return output_path
    ...
    def validate_package(
        ...
        if size_mb > 50:
            print(f"⚠️  Package size ({size_mb:.2f} MB) exceeds 50MB limit for direct upload")
