...
class CostAnalyzer:
    ...
    def get_project_costs(
        ...
        try:
            ...
            response = self.ce.get_cost_and_usage(
                ...
                Filter={"Tags": {"Key": "Project", "Values": [self.project_name]}},
                GroupBy=[
    ...
    def get_resource_costs(
        ...
        resource_costs = {
            "lambda": self._analyze_lambda_costs(
                resources.get("lambda", []), environment
            ),
            "dynamodb": self._analyze_dynamodb_costs(
                resources.get("dynamodb", []), environment
            ),
            "s3": self._analyze_s3_costs(resources.get("s3", []), environment),
            "cloudfront": self._analyze_cloudfront_costs(
                resources.get("cloudfront", []), environment
            ),
            "api_gateway": self._analyze_apigateway_costs(
                resources.get("apigateway", []), environment
            ),
        }
    ...
    def get_cost_forecast(self, days: int = 30) -> Dict[str, Any]:
        ...
        forecast = {
            ...
            "projected_cost": round(
                avg_daily_cost * days * (1 + weekly_trend * days / 7), 2
            ),
    ...
    def _analyze_lambda_costs(
        ...
        for function in functions:
            ...
            try:
                ...
                compute_cost = (
                    invocations * duration / 1000 * 512 / 1024
                ) * 0.0000166667
    ...
    def _analyze_dynamodb_costs(
        self, tables: List[str], environment: str
    ) -> Dict[str, float]:
    ...
    def _analyze_s3_costs(
        self, buckets: List[str], environment: str
    ) -> Dict[str, float]:
        """Analyze S3 bucket costs."""
        # Simplified - would use actual metrics
        return {bucket: 0.023 for bucket in buckets}  # Storage cost estimate

    def _analyze_cloudfront_costs(
        self, distributions: List[str], environment: str
    ) -> Dict[str, float]:
        """Analyze CloudFront distribution costs."""
        # Simplified - would use actual metrics
        return {dist: 0.085 for dist in distributions}  # Data transfer estimate

    def _analyze_apigateway_costs(
        self, apis: List[str], environment: str
    ) -> Dict[str, float]:
    ...
    def _get_metric_sum(
        self, namespace: str, metric_name: str, dimensions: Dict[str, str]
    ) -> float:
