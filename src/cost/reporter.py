...
class CostReporter:
    ...
    def generate_comparison_report(
        self, environments: Optional[List[str]] = None, days: int = 30
    ) -> str:
        """
    ...
    def generate_executive_summary(
        self, quarter: Optional[int] = None, year: Optional[int] = None
    ) -> str:
        """
        ...
        quarterly_costs = self.analyzer.get_project_costs(
            start_date, end_date, "MONTHLY"
        )
        total_cost = quarterly_costs["total_cost"]

        # Calculate cost trends
        monthly_costs = quarterly_costs.get(
            "daily_costs", []
        )  # Actually monthly when granularity=MONTHLY
...
"""
        ...
        # Add top 5 services
        services = sorted(
            quarterly_costs["services"].items(), key=lambda x: x[1], reverse=True
        )[:5]
...
"""
        ...
        for service, cost in sorted(
            service_breakdown.items(), key=lambda x: x[1], reverse=True
        ):
            percentage = (
                (cost / cost_data["total_cost"] * 100)
...
"""
        ...
        for env, status in budget_status.items():
            if "error" not in status:
                status_class = (
                    ...
                    else "warning" if status.get("status") == "WARNING" else "error"
                )
                html += f"""        <tr>
            <td>{env}</td>
    ...
    def _generate_comparison_text(
        self, env_data: Dict[str, Dict[str, Any]], days: int
    ) -> str:
...
"""
        ...
        for env, data in env_data.items():
            top_service = (
                max(data["services"].items(), key=lambda x: x[1])[0]
                if data["services"]
                else "N/A"
            )
    ...
    def _generate_recommendations(
        self, cost_data: Dict[str, Any], forecast: Dict[str, Any]
    ) -> List[str]:
        ...
        top_services = sorted(
            cost_data["services"].items(), key=lambda x: x[1], reverse=True
        )[:3]

        for service, cost in top_services:
            if "Lambda" in service and cost > 100:
                recommendations.append(
                    f"Review Lambda function efficiency - {service} costs ${cost:.2f}"
                )
            elif "S3" in service and cost > 50:
                recommendations.append(
                    f"Consider S3 lifecycle policies - {service} costs ${cost:.2f}"
                )
            elif "CloudFront" in service and cost > 100:
                recommendations.append(
                    f"Optimize CloudFront caching - {service} costs ${cost:.2f}"
                )
        ...
        if trend > 10:
            recommendations.append(
                f"Cost trend increasing at {trend:.1f}% weekly - investigate root cause"
            )

        # Generic recommendations
        if not recommendations:
            recommendations.append(
                "Continue monitoring costs for optimization opportunities"
            )
