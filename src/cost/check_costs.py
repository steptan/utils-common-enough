...
class ProjectCostChecker:
    ...
    def get_costs(self, days: int = 7, budget: Optional[float] = None) -> Dict:
        ...
        try:
            ...
            for result in response["ResultsByTime"]:
                ...
                daily_costs.append(
                    {"date": result["TimePeriod"]["Start"], "cost": daily_cost}
                )
            ...
            print("\nTroubleshooting:")
            print(
                f"  1. Ensure your AWS resources are tagged with Project={self.project_name}"
            )
    ...
    def _get_cost_filter(self) -> Dict:
        ...
        return {"Tags": {"Key": "Project", "Values": [self.project_name]}}
    ...
    def _check_monthly_budget(self, budget: Optional[float]) -> None:
        ...
        try:
            ...
            if budget:
                ...
                if percentage > 100:
                    print(
                        f"🚨 BUDGET EXCEEDED: {percentage:.0f}% of ${budget:.0f} monthly budget"
                    )
                elif percentage > 80:
                    print(
                        f"⚠️  Budget Warning: {percentage:.0f}% of ${budget:.0f} monthly budget"
                    )
                else:
                    print(
                        f"✅ Budget Status: {percentage:.0f}% of ${budget:.0f} monthly budget"
                    )
        except Exception as e:
                    ...
                    if self.project_name.lower() in func_name.lower():
                        try:
                            ...
                            if tags.get("Project") != self.project_name:
                                untagged.append(
                                    f"Lambda: {func_name} (missing/incorrect Project tag)"
                                )
                        except Exception:
                            pass
        except Exception as e:
                ...
                for table_name in page.get("TableNames", []):
                    if self.project_name.lower() in table_name.lower():
                        try:
                            ...
                            if tags.get("Project") != self.project_name:
                                untagged.append(
                                    f"DynamoDB: {table_name} (missing/incorrect Project tag)"
                                )
                        except Exception:
                            pass
        except Exception as e:
            ...
            for bucket in buckets_response.get("Buckets", []):
                ...
                if self.project_name.lower() in bucket_name.lower():
                    try:
                        ...
                        if tags.get("Project") != self.project_name:
                            untagged.append(
                                f"S3: {bucket_name} (missing/incorrect Project tag)"
                            )
...
def main():
    ...
    parser.add_argument(
        "--region", "-r", default="us-west-1", help="AWS region (default: us-west-1)"
    )
    parser.add_argument(
        "--budget", "-b", type=float, help="Monthly budget for comparison"
    )
    parser.add_argument(
        "--check-tags", "-t", action="store_true", help="Check for untagged resources"
    )
    parser.add_argument(
    ...
    try:
        ...
        checker = ProjectCostChecker(
            project_name=project_name, region=args.region, profile=args.aws_profile
        )
