...
class CostEstimator:
    ...
    def estimate_stack_cost(
        self, template_path: str, parameters: Optional[Dict[str, str]] = None
    ) -> Dict[str, Any]:
    ...
    def estimate_application_cost(
        self, usage_profile: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _estimate_resource(
        self, logical_id: str, resource_type: str, properties: Dict[str, Any]
    ...
    def _estimate_dynamodb_table(
        ...
        if billing_mode == "PAY_PER_REQUEST":
            ...
            storage_cost = (
                max(0, 10 - pricing["free_storage"]) * pricing["storage"]
            )  # 10GB assumed
            ...
            rcu = properties.get("ProvisionedThroughput", {}).get(
                "ReadCapacityUnits", 5
            )
            wcu = properties.get("ProvisionedThroughput", {}).get(
        ...
        return ResourceEstimate(
            ...
            cost_factors={"billing_mode": billing_mode, "storage_gb": 10},
    ...
    def _estimate_s3_bucket(
        ...
        # Calculate costs
        storage_cost = (
            max(0, storage_gb - pricing["free_storage"]) * pricing["storage_standard"]
        )
        put_cost = (put_requests / 1000) * pricing["requests_put"]
        get_cost = (
            max(0, get_requests - pricing["free_requests"])
            / 1000
            * pricing["requests_get"]
        )
    ...
    def _estimate_cloudfront(
        ...
        # Apply free tier
        billable_transfer = max(
            0, data_transfer_gb - pricing["free_data_transfer"] / 12
        )
        ...
        return ResourceEstimate(
            ...
            monthly_cost_min=round(total_cost, 2),
            monthly_cost_max=round(total_cost * 2, 2),
            cost_factors={"data_transfer_gb": data_transfer_gb, "requests": requests},
            assumptions=[
                f"Assumed {data_transfer_gb}GB data transfer",
    ...
    def _estimate_cognito(
        ...
        if billable_mau == 0:
            ...
            cost = (
                50_000 * pricing["mau_tier1"]
                + (billable_mau - 50_000) * pricing["mau_tier2"]
            )
        ...
        return ResourceEstimate(
            ...
            cost_factors={"monthly_active_users": mau},
            assumptions=[
    ...
    def _estimate_nat_gateway(
        ...
        return ResourceEstimate(
            ...
            monthly_cost_min=round(total_cost, 2),
            monthly_cost_max=round(total_cost * 1.5, 2),
            cost_factors={"hours": hours, "data_processed_gb": data_processed_gb},
            assumptions=[
                "24/7 operation",
    ...
    def _estimate_api_costs(self, usage: Dict[str, Any]) -> List[ResourceEstimate]:
        ...
        lambda_request_cost = (billable_requests / 1_000_000) * lambda_pricing[
            "request_price"
        ]
        ...
        estimates.append(
            ResourceEstimate(
                ...
                monthly_cost_max=round(
                    (lambda_request_cost + lambda_compute_cost) * 2, 2
                ),
                cost_factors={
                    "memory_mb": memory,
    ...
    def _estimate_database_costs(self, usage: Dict[str, Any]) -> ResourceEstimate:
        ...
        return ResourceEstimate(
            ...
            monthly_cost_min=round(total_cost, 2),
            monthly_cost_max=round(total_cost * 2, 2),
            cost_factors={"reads": reads, "writes": writes, "storage_gb": storage},
            assumptions=["On-demand billing mode", f"{storage}GB total storage"],
        )
    ...
    def _estimate_storage_costs(self, usage: Dict[str, Any]) -> ResourceEstimate:
        ...
        storage_cost = (
            max(0, storage - pricing["free_storage"]) * pricing["storage_standard"]
        )
        put_cost = (uploads / 1000) * pricing["requests_put"]
        get_cost = (
            max(0, downloads - pricing["free_requests"])
            / 1000
            * pricing["requests_get"]
        )
        ...
        return ResourceEstimate(
            ...
            assumptions=["Standard storage class", "Minimal data transfer costs"],
        )
    ...
    def _estimate_cdn_costs(self, usage: Dict[str, Any]) -> ResourceEstimate:
        ...
        return ResourceEstimate(
            ...
            monthly_cost_min=round(total_cost, 2),
            monthly_cost_max=round(total_cost * 2, 2),
            cost_factors={"traffic_gb": traffic, "requests": requests},
            assumptions=["US/EU traffic pricing", "HTTPS requests only"],
        )
    ...
    def _estimate_monitoring_costs(self, usage: Dict[str, Any]) -> ResourceEstimate:
        ...
        log_ingestion_cost = (
            max(0, log_gb - pricing["free_logs_gb"]) * pricing["logs_ingestion"]
        )
    ...
    def _group_by_service(
        ...
        for service in by_service:
            by_service[service]["monthly_min"] = round(
                by_service[service]["monthly_min"], 2
            )
            by_service[service]["monthly_max"] = round(
                by_service[service]["monthly_max"], 2
            )
        ...
        for estimate in estimates:
            if estimate.monthly_cost_max > 100:
                if estimate.resource_type == ResourceType.NAT_GATEWAY:
                    tips.append(
                        "Consider using VPC endpoints instead of NAT Gateway for S3/DynamoDB access"
                    )
                    ...
                    tips.append(
                        "Consider DynamoDB auto-scaling or on-demand for variable workloads"
                    )
