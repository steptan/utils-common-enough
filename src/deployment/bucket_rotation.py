...
class BucketRotationManager:
    ...
    def __init__(self, project_name: str, environment: str, region: str, account_id: str):
        """
    ...
    def _parse_bucket_name(self, bucket_name: str) -> Optional[Tuple[str, str, int, int]]:
        """
    ...
    def list_project_buckets(self) -> List[Dict[str, any]]:
        ...
        try:
            ...
            for bucket in response.get('Buckets', []):
                ...
                if parsed and parsed[0] == self.project_name and parsed[1] == self.environment:
    ...
    def cleanup_old_buckets(self) -> List[str]:
        ...
        if len(buckets) <= self.RETENTION_COUNT:
            logger.info(f"Only {len(buckets)} buckets exist, keeping all (retention: {self.RETENTION_COUNT})")
            return deleted
