...
class FrontendDeployer(BaseDeployer):
        ...
        # For Next.js with .next/static
            ...
            if next_static.exists():
                for file_path in next_static.rglob("*"):
                    if file_path.is_file():
                        s3_key = "_next/static/" + str(file_path.relative_to(next_static))
                        files.append((file_path, s3_key))
    ...
    def deploy(self) -> DeploymentResult:
        ...
        self.log(f"Deploying {self.project_name} frontend to {self.environment}", "INFO")
        ...
        if not frontend_bucket:
            ...
            self.add_warning(f"Frontend bucket not in stack outputs, using: {frontend_bucket}")
        ...
        if distribution_id:
            if not self.invalidate_cloudfront(distribution_id):
                ...
                self.add_warning("CloudFront invalidation failed, content may be cached")
        ...
        if distribution_id:
            ...
            self.add_output("FrontendURL", f"http://{frontend_bucket}.s3-website{region_part}.amazonaws.com")
