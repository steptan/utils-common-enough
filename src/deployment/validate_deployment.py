...
class DeploymentValidator:
    ...
    def test_api_detailed_health(self) -> bool:
        ...
        try:
            response = requests.get(
                f"{self.api_url}/health/detailed",
                timeout=10
            )
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.add_result(
                    "Website",
                    False,
                    f"Unavailable - Status: {response.status_code}"
                )
                return False
    ...
    def test_api_cors(self) -> bool:
        ...
        try:
            ...
            has_cors = all(
                h in response.headers for h in cors_headers
            )
            
            self.add_result(
                "API CORS",
                has_cors,
                "Configured" if has_cors else "Not configured"
            )
            return has_cors
    ...
    def test_cloudfront_headers(self) -> bool:
        ...
        try:
            ...
            self.add_result(
                "CloudFront",
                has_cf,
                f"Active - {len(cf_headers)} CF headers found" if has_cf else "Not detected"
            )
            return has_cf
...
def main():
    ...
    parser.add_argument(
        "outputs_file",
        help="Path to deployment outputs JSON file"
    )
    parser.add_argument(
        "--wait",
        action="store_true",
