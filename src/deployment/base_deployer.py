...
class BaseDeployer(ABC):
    ...
    def clean_failed_stack(self, stack_name: str) -> bool:
        ...
        if status in [
            ...
            self.log(
                f"Stack {stack_name} is in {status} state. Cleaning up...", "WARNING"
            )
            ...
            if not self.dry_run:
                try:
                    ...
                    self.log(
                        f"Successfully deleted failed stack {stack_name}", "SUCCESS"
                    )
                    return True
                except Exception as e:
    ...
    def create_s3_bucket_if_needed(
        self, bucket_name: str, versioning: bool = True
    ) -> bool:
    ...
    def wait_for_stack(
        self, stack_name: str, operation: str = "create", max_attempts: int = 120
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                for event in response["StackEvents"]:
                    ...
                    if (
                        "FAILED" in status
                        and "Resource creation cancelled"
                        not in event.get("ResourceStatusReason", "")
                    ):
