...
class FraudOrNotDeployer(InfrastructureDeployer):
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        if not base_config_path.exists():
            ...
            base_config_path = Path(__file__).parent.parent.parent / 'config' / 'fraud-or-not.yaml'
        
        with open(base_config_path, 'r') as f:
            config = yaml.safe_load(f)
            
        # Load environment-specific config
        env_config_path = self.project_root / 'config' / 'environments' / f'{self.environment}.yaml'
    ...
    def deploy(self, dry_run: bool = False, auto_approve: bool = False) -> bool:
        ...
        self.logger.info(f"Starting deployment for {self.project_name}-{self.environment}")
        
        # Generate template
        template = self.generate_template()
        
        # Save template
        template_path = self.project_root / 'deployments' / self.environment / 'template.json'
        ...
        try:
            ...
            if existing_stack:
                if not auto_approve:
                    response = input(f"Stack {stack_name} already exists. Update? (y/N): ")
                    if response.lower() != 'y':
            ...
            if final_stack and 'Outputs' in final_stack:
                ...
                for output in final_stack['Outputs']:
                    self.logger.info(f"  {output['OutputKey']}: {output['OutputValue']}")
