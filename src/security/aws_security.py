    ...
    def validate_credentials(self) -> Tuple[bool, Optional[Dict[str, Any]]]:
        ...
        try:
            ...
            return True, {
                ...
                "is_temporary": "assumed-role" in user_arn
                or "federated-user" in user_arn,
            }
    ...
    def enforce_security_policies(self, environment: str) -> Tuple[bool, List[str]]:
        ...
        if environment == "prod":
            if identity["credential_type"] == "iam_user":
                issues.append(
                    "⚠️  WARNING: Using long-term IAM user credentials in production"
                )
                # Check for MFA
                if not identity["is_mfa_enabled"]:
                    issues.append(
                        "❌ CRITICAL: MFA is required for production deployments"
                    )
                    return False, issues
            ...
            if not identity["is_temporary"]:
                issues.append(
                    "⚠️  RECOMMENDATION: Use temporary credentials (STS) for production"
                )
