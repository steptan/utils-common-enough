...
class SmokeTestRunner:
    ...
    def __init__(
        ...
        if not base_url or not api_url:
            ...
            self.base_url = base_url or outputs.get("CloudFrontUrl", outputs.get("FrontendURL", ""))
    ...
    def _run_test(self, test_func) -> TestResult:
        ...
        print(f"{status_emoji} {test_name}: {test_result.message} ({test_result.duration:.2f}s)")
    ...
    def test_homepage(self) -> Optional[Tuple[TestStatus, str]]:
        ...
        if self.project_name == "fraud-or-not" and "fraud" not in content:
            ...
            return TestStatus.WARNING, "Homepage loaded but missing expected content"
        elif self.project_name == "people-cards" and ("card" not in content and "people" not in content):
            return TestStatus.WARNING, "Homepage loaded but missing expected content"
        
        return TestStatus.PASSED, f"Homepage loaded successfully ({len(response.content)} bytes)"
        ...
        for endpoint in health_endpoints:
            try:
                ...
                if response.status_code in [200, 401, 403]:  # 401/403 means API is up but needs auth
                    return TestStatus.PASSED, f"API responded at {endpoint} ({response.status_code})"
            except:
                continue
    ...
    def test_security_headers(self) -> Optional[Tuple[TestStatus, str]]:
        ...
        if not missing:
            ...
            return TestStatus.WARNING, f"Missing some security headers: {', '.join(missing)}"
        else:
    ...
    def test_media_register_specific(self) -> Optional[Tuple[TestStatus, str]]:
        ...
        try:
            response = requests.get(f"{self.api_url}/api/media-types", timeout=self.timeout)
            if response.status_code == 200:
    ...
    def test_fraud_submission_endpoint(self) -> Optional[Tuple[TestStatus, str]]:
        ...
        try:
            response = requests.options(f"{self.api_url}/api/submit", timeout=self.timeout)
    ...
    def test_cards_api(self) -> Optional[Tuple[TestStatus, str]]:
        ...
        try:
            ...
            if response.status_code in [200, 401, 403]:  # May require auth
                return TestStatus.PASSED, f"Cards API responding ({response.status_code})"
        except:
            pass
