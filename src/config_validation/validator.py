...
for tag in cfn_tags:
    CloudFormationYAMLLoader.add_constructor(
        f"!{tag}", lambda loader, node, tag=tag: cfn_tag_constructor(loader, tag, node)
    )
...
class ConfigurationValidator:
    ...
    def _find_config_dir(self) -> Path:
        ...
        raise FileNotFoundError(
            f"Could not find config directory for {self.project_name}"
        )
        ...
        def deep_merge(
            ...
            for key, value in override.items():
                ...
                if key in result:
                    ...
                    if (
                        current_path == "storage.dynamodb.tables"
        ...
        try:
            ...
            if env_config_path.exists():
                ...
                result["warnings"].append(
                    f"No environment-specific configuration found for {environment}"
                )
