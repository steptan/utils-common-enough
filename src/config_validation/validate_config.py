...
def validate_environment_config(
    environment: str, config_dir: str = "config"
) -> List[str]:
    """
...
def _validate_compute_config(config: Dict[str, Any]) -> List[str]:
    ...
    if runtime and runtime not in valid_runtimes:
        errors.append(
            f"Invalid Lambda runtime: {runtime}. Must be one of {valid_runtimes}"
        )
    ...
    if timeout and (timeout < 1 or timeout > 900):
        errors.append(
            f"Invalid Lambda timeout: {timeout}. Must be between 1 and 900 seconds"
        )
    ...
    if memory_size:
        if memory_size < 128 or memory_size > 10240:
            errors.append(
                f"Invalid Lambda memory size: {memory_size}. Must be between 128 and 10240 MB"
            )
        if memory_size % 64 != 0:
            errors.append(
                f"Invalid Lambda memory size: {memory_size}. Must be a multiple of 64 MB"
            )
...
def _validate_environment_specific(
    ...
    if environment == "prod":
        ...
        if not dynamodb.get("deletion_protection", False):
            errors.append(
                "Production DynamoDB tables must have deletion protection enabled"
            )
