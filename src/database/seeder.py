...
class DataSeeder:
    ...
    def get_table_name(self, table_key: str) -> str:
        ...
        if hasattr(self.config, 'custom_config') and 'table_patterns' in self.config.custom_config:
        ...
        if hasattr(self.config, 'table_patterns') and table_key in self.config.table_patterns:
    ...
    def clear_table(self, table_key: str, confirm: bool = False) -> bool:
        ...
        try:
            ...
            with table.batch_writer() as batch:
                for item in items:
                    # Build key dict
                    key_dict = {attr: item[attr] for attr in key_attributes if attr in item}
    ...
    def clear_all_tables(self, table_keys: List[str], confirm: bool = False) -> bool:
        ...
        """
        if not confirm:
            response = input(f"⚠️  Clear all data from {len(table_keys)} tables in {self.environment}? [y/N]: ")
            if response.lower() != 'y':
                print("Cancelled")
                return False
    ...
    def _process_item(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in item.items():
            if isinstance(value, datetime):
                ...
                # Process list items
                processed[key] = [
                    self._process_item(v) if isinstance(v, dict) else v
                    for v in value
                ]
            else:
                processed[key] = value
    ...
    def generate_sample_data(self) -> SeedData:
        ...
        """
        self._print_warning("Using generic sample data - override generate_sample_data() for project-specific data")
        
        seed_data = SeedData()
        
        # Generic sample data
        seed_data.add_items("users", [
            {
                "id": "user-1",
                "name": "Test User 1",
        ...
        for i, pol in enumerate(politicians):
            for j in range(6):  # 6 actions per politician
                ...
                actions.append({
                    ...
                    'category': ['legislation', 'vote', 'statement', 'meeting'][j % 4],
