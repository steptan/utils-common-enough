...
class StackManager:
    ...
    def diagnose_stack_failure(self, stack_name: str) -> Dict[str, Any]:
        ...
        if status == "DELETE_FAILED":
            ...
            try:
                resources = self.cloudformation.describe_stack_resources(StackName=stack_name)
    ...
    def _get_failure_recommendations(self, resource_type: str, reason: str) -> List[str]:
        ...
        if resource_type == "AWS::S3::Bucket":
            if "BucketNotEmpty" in reason or "bucket is not empty" in reason.lower():
                ...
                recommendations.append("S3 bucket name already exists. Choose a different name.")
        
        # Lambda ENI issues
        if "AWS::EC2::NetworkInterface" in reason and "Lambda" in reason:
            recommendations.append("Lambda ENIs can take time to delete. Wait 10-15 minutes.")
        ...
        if "AccessDenied" in reason or "is not authorized" in reason:
            ...
            recommendations.append("Ensure the deployment role has necessary permissions")
        
        # VPC issues
        if resource_type.startswith("AWS::EC2::") and "DependencyViolation" in reason:
            recommendations.append("VPC resources have dependencies. Check security groups and ENIs.")
        
        # General timeout issues
        if "timeout" in reason.lower():
            recommendations.append("Operation timed out. Check resource logs for details.")
            if "Lambda" in resource_type:
                recommendations.append("Check Lambda function logs in CloudWatch")
        
        return recommendations
    
    def fix_rollback_state(self, stack_name: str, skip_resources: Optional[List[str]] = None) -> bool:
        """Fix a stack in ROLLBACK_COMPLETE or ROLLBACK_FAILED state."""
        status = self.get_stack_status(stack_name)
        
        if status not in ["ROLLBACK_COMPLETE", "ROLLBACK_FAILED", "UPDATE_ROLLBACK_FAILED"]:
    ...
    def _handle_delete_blockers(self, stack_name: str) -> None:
        ...
        try:
            ...
            for resource in response["StackResources"]:
                # Handle S3 buckets
                if resource["ResourceType"] == "AWS::S3::Bucket" and resource["ResourceStatus"] == "DELETE_FAILED":
            ...
            for page in paginator.paginate(
                ...
                for stack in page["StackSummaries"]:
                    ...
                    stacks.append({
                        ...
                        "updated": str(stack.get("LastUpdatedTime", stack["CreationTime"]))
                    })
    ...
    def get_cognito_config(self, stack_name: str) -> Dict[str, str]:
        ...
        cognito_keys = {
            ...
            "UserPoolClientId": ["UserPoolClientId", "CognitoClientId", "AppClientId", "ClientId"],
    ...
    def get_api_endpoints(self, stack_name: str) -> Dict[str, str]:
        ...
        api_keys = [
            "ApiEndpoint", "ApiUrl", "RestApiEndpoint", "ApiGatewayUrl",
            "GraphQLEndpoint", "WebSocketEndpoint", "HttpApiEndpoint"
        ]
    ...
    def get_s3_buckets(self, stack_name: str) -> Dict[str, str]:
        ...
        bucket_keys = [
            "Bucket", "S3Bucket", "StorageBucket", "AssetsBucket",
            "DeploymentBucket", "WebsiteBucket", "StaticBucket"
        ]
    ...
    def get_database_config(self, stack_name: str) -> Dict[str, Any]:
        ...
        table_keys = [
            "TableName", "DynamoDBTable", "DynamoTable",
            "MainTable", "DataTable"
        ]
    ...
    def validate_stack_template(self, template_body: Optional[str] = None, 
                               template_url: Optional[str] = None) -> Dict[str, Any]:
