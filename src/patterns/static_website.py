...
from troposphere import (
    Template, Output, Ref, GetAtt, Sub, Export,
    Join, If, Equals, Not, Condition
)
...
class StaticWebsitePattern:
    ...
    def _create_conditions(self):
        ...
        self.has_custom_domain = self.template.add_condition(
            "HasCustomDomain",
            Not(Equals(
                self.domain_config.get("domain_name", ""),
                ""
            ))
        )
        ...
        self.has_certificate = self.template.add_condition(
            "HasCertificate",
            Not(Equals(
                self.domain_config.get("certificate_arn", ""),
                ""
            ))
        )
        ...
        self.website_bucket = self.template.add_resource(
            s3.Bucket(
                ...
                VersioningConfiguration=s3.VersioningConfiguration(
                    Status="Enabled" if self.pattern_config.get("enable_versioning", True) else "Suspended"
                ),
                ...
                ) if self.pattern_config.get("enable_versioning", True) else Ref("AWS::NoValue"),
                Tags=[
                    {"Key": "Name", "Value": Sub(f"${{AWS::StackName}}-website-bucket")},
    ...
    def _create_pattern_outputs(self):
        ...
        self.template.add_output(Output(
            "CloudFrontURL",
            Value=Sub("https://${Domain}", Domain=GetAtt(self.distribution, "DomainName")),
    ...
    def get_default_config(environment: str = "dev") -> Dict[str, Any]:
        ...
        return {
            ...
            "cloudfront": {
                "price_class": "PriceClass_100" if environment != "prod" else "PriceClass_All",
                "min_ttl": 0,
                "default_ttl": 86400,
                "max_ttl": 31536000
            },
    ...
    def validate_config(config: Dict[str, Any]) -> List[str]:
        ...
        if "cloudfront" in config:
            ...
            if cf.get("price_class") and cf["price_class"] not in valid_price_classes:
                errors.append(f"cloudfront.price_class must be one of: {valid_price_classes}")
            ...
            for ttl_key in ["min_ttl", "default_ttl", "max_ttl"]:
                if ttl_key in cf:
                    ...
                    if not isinstance(ttl_value, int) or ttl_value < 0:
                        errors.append(f"cloudfront.{ttl_key} must be a non-negative integer")
        
        # Validate domain configuration
        if "domain" in config and config["domain"].get("create_dns_records", False):
            domain = config["domain"]
            if not domain.get("domain_name"):
                errors.append("domain.domain_name is required when create_dns_records is True")
            if not domain.get("hosted_zone_id"):
                errors.append("domain.hosted_zone_id is required when create_dns_records is True")
