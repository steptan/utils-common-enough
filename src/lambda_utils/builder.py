...
class LambdaBuilder:
    ...
    def build_function(
        ...
        if self.runtime.startswith("nodejs"):
            return self._build_nodejs_function(source_dir, function_output, function_name)
        elif self.runtime.startswith("python"):
            return self._build_python_function(source_dir, function_output, function_name)
        else:
            ...
            if shutil.which("docker"):
                ...
                subprocess.run(
                    [
                        ...
                        "-t", str(output_dir),
                        "--no-deps",
                        "--platform", "manylinux2014_x86_64" if self.architecture == "x86_64" else "manylinux2014_aarch64",
                        "--only-binary", ":all:"
                    ],
                    check=True,
                    capture_output=True
    ...
    def _build_python_docker(self, requirements_file: Path, output_dir: Path) -> None:
        ...
        subprocess.run(
            [
                "docker", "run",
                "--rm",
                "-v", f"{requirements_file.parent}:/var/task",
    ...
    def _is_lambda_function(self, directory: Path) -> bool:
        ...
        # Node.js indicators
        if self.runtime.startswith("nodejs"):
            return (directory / "index.js").exists() or (directory / "index.ts").exists()
        
        # Python indicators
        elif self.runtime.startswith("python"):
            return (directory / "lambda_function.py").exists() or (directory / "handler.py").exists()
