...
from troposphere import (
    ...
    Parameter, Export, ImportValue, Join, If, Equals
)
    ...
    def _create_dynamodb_tables(self):
        ...
        for table_config in tables_config:
            ...
            if "name_pattern" in table_config:
                ...
                full_table_name = Sub(f"${{AWS::StackName}}-{table_name}-{self.environment}")
            ...
            # Partition key
            partition_key = table_config.get("partition_key", {"name": "id", "type": "S"})
            key_schema.append(
                dynamodb.KeySchema(
            ...
            for gsi_config in table_config.get("global_secondary_indexes", []):
                ...
                for key_type in ["partition_key", "sort_key"]:
                    if key_type in gsi_config:
                        ...
                        if not any(attr.AttributeName == key_info["name"] for attr in attribute_definitions):
                            attribute_definitions.append(
                                dynamodb.AttributeDefinition(
                ...
                if table_config.get("billing_mode", "PAY_PER_REQUEST") != "PAY_PER_REQUEST":
            ...
            if table_config.get("point_in_time_recovery", True):
                table.PointInTimeRecoverySpecification = dynamodb.PointInTimeRecoverySpecification(
                    PointInTimeRecoveryEnabled=True
                )
            
            # Enable server-side encryption
            if table_config.get("encryption", True):
                table.SSESpecification = dynamodb.SSESpecification(
                    SSEEnabled=True
                )
    ...
    def _create_s3_buckets(self):
        ...
        for bucket_config in buckets_config:
                    ...
                    if "transition_days" in rule_config:
                        rule.Transitions = [
                            s3.LifecycleRuleTransition(
                                ...
                                StorageClass=rule_config.get("storage_class", "GLACIER")
                            )
                        ]
            ...
            if bucket_config.get("block_public_access", True):
                bucket.PublicAccessBlockConfiguration = s3.PublicAccessBlockConfiguration(
            ...
            if "cors_rules" in bucket_config:
                ...
                bucket.CorsConfiguration = s3.CorsConfiguration(
