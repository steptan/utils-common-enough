...
class DistributionConstruct:
    ...
    def _create_s3_bucket(self):
        ...
        self.s3_bucket = self.template.add_resource(
            s3.Bucket(
                "StaticAssetsBucket",
                BucketName=Sub(
                    f"people-cards-static-{self.environment}-${{AWS::AccountId}}"
                ),
    ...
    def _create_cloudfront_distribution(self):
        ...
        s3_origin = cloudfront.Origin(
            ...
            S3OriginConfig=cloudfront.S3OriginConfig(
                OriginAccessIdentity=Join(
                    "", ["origin-access-identity/cloudfront/", Ref(self.oai)]
                )
            ),
        )
        ...
        default_cache_behavior = cloudfront.DefaultCacheBehavior(
            ...
            ViewerProtocolPolicy=cf_config.get("default_cache_behavior", {}).get(
                "viewer_protocol_policy", "redirect-to-https"
            ),
            AllowedMethods=cf_config.get("default_cache_behavior", {}).get(
                "allowed_methods", ["GET", "HEAD"]
            ),
            CachedMethods=["GET", "HEAD"],
            Compress=cf_config.get("default_cache_behavior", {}).get("compress", True),
            CachePolicyId=cf_config.get("default_cache_behavior", {}).get(
        ...
        if cf_config.get("enable_logging", False):
            ...
            logging_bucket = self.template.add_resource(
                s3.Bucket(
                    ...
                    BucketName=Sub(
                        f"people-cards-cf-logs-{self.environment}-${{AWS::AccountId}}"
                    ),
                    PublicAccessBlockConfiguration=s3.PublicAccessBlockConfiguration(
                        BlockPublicAcls=True,
                        BlockPublicPolicy=True,
    ...
    def _create_outputs(self):
        ...
        outputs = {
            ...
            "DistributionURL": {
                "value": Join(
                    "", ["https://", GetAtt(self.distribution, "DomainName")]
                ),
